 ...
 */
const cellIsActive = (cellId) => cells[parseInt(cellId)].getAttribute('cell-status') !== 'deactivated';
...
const addCell = (row, innerHTML, counter) => {
  ...
  cellNum = innerHTML.toString();
  if (cellNum.length > 2) {
    cellNum = cellNum.split('<')[0];
  }
  ...
  // Add tile image if not already added
  if (col.childElementCount === 0) {
    var tileImg = document.createElement('img');
    tileImg.src = `assets/tiles/${innerHTML}.svg`;
    col.appendChild(tileImg);
  }

  // Set tile attr
  col.setAttribute('class', 'gameTable');
  col.setAttribute('cell-value', cellNum);
